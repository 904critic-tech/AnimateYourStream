...
export default function SandboxModelViewer({ onModelLoaded }: SandboxModelViewerProps) {
  ...
  // 🎭 Agent 4 - Fix: Clear any uploaded models that might interfere with character loading
  useEffect(() => {
    // Clear any uploaded models to ensure proper character loading for lip sync testing
    if (currentUploadedModel) {
      console.log('🎭 Agent 4 - Clearing uploaded model to ensure proper character loading:', currentUploadedModel.name)
      clearUploadedModels()
    ...
    if (!currentModel) {
      console.log('🎭 Agent 4 - No current model set, defaulting to Elmo for lip sync testing')
      // The store should already default to 'elmo', but let's make sure
    } else {
      console.log('🎭 Agent 4 - Current model for lip sync testing:', currentModel)
    }
  ...
  const loadModel = useCallback(async (url: string, name?: string) => {
    try {
      ...
      if (lastLoadedUrlRef.current === url) {
        console.log('🎭 Sandbox: Skipping duplicate load for URL')
        return
      }
      lastLoadedUrlRef.current = url
      const myLoadId = ++loadIdRef.current
      console.log('🎭 Sandbox: Loading model:', url)
      ...
      if (extension === 'fbx' && fbxLoader.current) {
        model = await new Promise<Group>((resolve, reject) => {
          fbxLoader.current!.load(
            ...
            undefined,
            (err) => {
              console.error('🎭 Sandbox: FBX load failed:', (err && (err as any).message) || String(err))
              reject(err)
            }
          )
        })
        ...
        try {
          ...
          console.error('🎭 Sandbox: GLTF load failed:', e?.message || e)
          throw e
        }
      } else {
      ...
      try {
        ...
        ;(model as any).updateMatrixWorld?.(true)
        console.log('🎭 Sandbox: Normalized model scale', { currentRadius, normalizeScale })
      } catch (e) {
        console.warn('🎭 Sandbox: Normalization failed, using default scale', e)
      }
      ...
      if (model.animations && model.animations.length > 0) {
        ...
        console.log('🎭 Sandbox: Found animations:', animations.map(a => a.name))
        console.log('🎭 Sandbox: Animation mixer created:', !!newMixer)
        } else {
        ...
        if (allAnimations.length > 0) {
          ...
          try {
            ...
            console.log('🎭 Sandbox: Retargeting map size:', Object.keys(map).length, 'remapped:', remapped.length)
          } catch (e) {
            console.warn('🎭 Sandbox: Retargeting failed, using original clips', e)
            newMixer = new AnimationMixer(model)
          ...
          setCurrentAnimations(animations)
          applyDefaultAnimationSelection(animations)
          console.log('🎭 Sandbox: Found animations in child objects:', animations.map(a => a.name))
          console.log('🎭 Sandbox: Animation mixer created from child animations:', !!newMixer)
        } else {
          // Check per-model procedural idle toggle
          const { currentModel, proceduralIdleSettings } = useAppStore.getState()
          const settings = (currentModel && proceduralIdleSettings[currentModel]) || { enabled: true }
          if (settings.enabled === false) {
            console.log('🎭 Sandbox: No animations found and procedural idle disabled')
            newMixer = new AnimationMixer(model)
            setMixer(newMixer)
          ...
          try {
            ...
            if (lib.length > 0 && fbxLoader.current) {
              ...
              for (const entry of lib) {
                try {
                  ...
                  console.log('🎭 Sandbox: Loaded external animation:', entry.name, 'tracks:', remapped.length || srcClips.length)
                } catch (e) {
                  console.warn('🎭 Sandbox: Failed to load external animation:', entry.path)
                }
              }
              if (loadedClips.length > 0) {
                ...
                setCurrentAnimations(animations)
                applyDefaultAnimationSelection(animations)
                console.log('🎭 Sandbox: External library loaded with clips:', animations.map(a => a.name))
              } else {
                // Fallback to procedural idle if library empty
                console.log('🎭 Sandbox: No external clips; generating procedural idle clip')
              ...
              console.log('🎭 Sandbox: No animations; generating procedural idle clip')
              const proceduralClip = createProceduralIdleClip(model)
              newMixer = new AnimationMixer(model)
              animations.push(proceduralClip)
              setMixer(newMixer)
            ...
            console.warn('🎭 Sandbox: External animation load failed, using procedural idle', e)
            const proceduralClip = createProceduralIdleClip(model)
            newMixer = new AnimationMixer(model)
            animations.push(proceduralClip)
            setMixer(newMixer)
      ...
      console.error('🎭 Sandbox: Failed to load model:', message)
  ...
  const placeMarkers = useCallback(() => {
    ...
    setGuidedIndex(0)
    setPlacing(true)
    console.log('🎭 Sandbox: Guided placement started. Place:', sequence[0])
  ...
  useEffect(() => {
    ...
    const onPointerDown = (ev: MouseEvent) => {
      ...
      if (guidedLabels && nextIndex >= guidedLabels.length) {
        setPlacing(false)
        console.log('🎭 Sandbox: Markers complete')
      } else {
        setGuidedIndex(nextIndex)
        console.log('🎭 Sandbox: Placed:', label, 'Next:', guidedLabels ? guidedLabels[nextIndex] : '')
      }
    }
  ...
  const buildSkeletonFromMarkers = useCallback(() => {
    try {
      ...
      if (!currentRoot) {
        console.log('🎭 Sandbox: No model to build skeleton for')
        return
      }
      ...
      if (missing.length > 0) {
        ...
        console.warn('🎭 Sandbox: Missing markers:', missing)
        return
      }
      ...
      console.log('🎭 Sandbox: Skeleton generated')
      
    } catch (e) {
      console.error('🎭 Sandbox: Skeleton generation failed:', e)
    }
  ...
  const bindSkeletonToCurrentModel = useCallback(() => {
    try {
      if (!currentRoot || !currentSkeleton || !Array.isArray(currentSkeleton.bones) || currentSkeleton.bones.length === 0) {
        console.log('🎭 Sandbox: bind skipped: missing model or skeleton')
        return
      }
      ...
      console.log('🎭 Sandbox: Auto weights bound')
      
    } catch (e) {
      console.error('🎭 Sandbox: bind failed:', e)
    }
  ...
  const saveGLB = useCallback(async () => {
    try {
      if (!currentRoot || !gltfExporter.current) {
        console.log('🎭 Sandbox: No model to export')
        return
      }
      ...
      console.log('🎭 Sandbox: GLB exported successfully')
      
    } catch (error) {
      console.error('🎭 Sandbox: GLB export failed:', error)
    }
  ...
  const clear = useCallback(() => {
    ...
    console.log('🎭 Sandbox: Scene cleared')
  ...
  useFrame((_, delta) => {
    if (mixer) {
      ...
      if (Math.random() < 0.001) { // Log very occasionally to avoid spam
        console.log('🎭 Sandbox: Mixer update - delta:', deltaClamped.toFixed(4), 'time:', mixer.time.toFixed(2))
      }
    }
  })
  ...
  useEffect(() => {
    if (currentUploadedModel && currentUploadedModel.url) {
      console.log('🎭 Sandbox: Loading uploaded blob:', currentUploadedModel.name)
  ...
  useEffect(() => {
    if (currentModel && !currentUploadedModel) {
      console.log('🎭 Sandbox: Loading character model:', currentModel)
  ...
  useEffect(() => {
    console.log('🎭 Sandbox: Animation status - mixer:', !!mixer, 'animations:', currentAnimations.length, 'names:', currentAnimations.map(a => a.name))
      ...
      {currentRoot && (
        <LipSync
          ...
          onLipSyncEvent={(event) => {
            console.log('🎭 Agent 4 - Lip sync event:', event)
          }}
          onPerformanceUpdate={(metrics) => {
            console.log('🎭 Agent 4 - Performance metrics:', metrics)
          }}
        />
      )}
    </group>
